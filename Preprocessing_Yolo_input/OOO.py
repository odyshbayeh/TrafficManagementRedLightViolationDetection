...
# get your primary monitor’s size
screen_w, screen_h = pyautogui.size()

# --- CSV SETUP ---
csv_file = open("polygons.csv", "w", newline="")
...
traffic_light_schedule = {
    'ID-1-F': [(1020, 2250), (4020, 5250), (7020, 8250), (10020, 11250),
               (13110, 14340), (16200, 17430), (19290, 20520), (22200, 23610),
               (25200, 26700)],
    'ID-1-L': [(1020, 1470), (4020, 4470), (7020, 7470), (10020, 10750),
               (13110, 13560), (16200, 16650), (19290, 19740), (22200, 22830),
               (25200, 25920)],
    'ID-2':   [(0, 360), (3000, 3360), (6000, 6360), (9000, 9360),
               (12000, 12390), (15000, 15480), (18050, 18460),
               (21000, 21500), (24100, 24450)],
    'ID-3-F': [(1620, 2850), (4620, 5850), (7620, 8850), (10900, 11850),
               (13710, 14850), (16800, 17900), (19890, 20850),
               (22980, 23950), (26070, 26850)],
    'ID-3-L': [(2400, 2850), (5400, 5850), (8400, 8850), (11400, 11850),
               (14490, 14850), (17580, 17900), (20670, 20850),
               (23760, 23950)],
    'ID-4':   [(510, 870), (3510, 3870), (6510, 6870), (9510, 9870),
               (12540, 12960), (15630, 16050), (18610, 19140),
               (21650, 22050), (24600, 25050)]
}
...
# show frames until user captures reference
...
print("Modes: 1-4=intersection | x/p=mask | v/d=crossing | s=start processing")
while True:
    tmp = ref_frame.copy()
    # draw intersections
...
while True:
    ...
    for lid, ctr in traffic_lights_positions.items():
        ...
        if lid.endswith("-F") or lid.endswith("-L"):
    ...
    # dynamic homography overlays & CSV logging
    if Hdyn is not None:
        for key, H in intersection_homographies.items():
            Htot = Hdyn.dot(H)
            poly = transform_points(world_intersections[key], Htot)

            # write polygon coords to CSV
            coords = poly.flatten().tolist()  # [x1,y1,...,x4,y4]
            csv_writer.writerow([frame_count, key] + coords)
                ...
                if n > 1e-5:
                    ...
                    if pn > 1e-5:
                        ...
                        if key in ("ID-1","ID-3"):
                            ...
                            st = get_light_state(key, frame_count)
                            linecol = base_green if st in ("green","yellow") else base_red
    ...
    # resize to fill screen and show
